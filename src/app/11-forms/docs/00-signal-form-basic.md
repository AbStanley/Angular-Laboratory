...
updateUsingForm() {
  ...
  this.form().value.set({ username: 'PXstanley', email: 'PXstanley@pxstanley.io' });
}
```
...
updateUsingSignal() {
  ...
  this.user.set({ username: 'Stanley', email: 'Stanley@pxstanley.io' });
}
```
...
@Component({
  ...
  template: `
    ...
    <button (click)="updateUsingSignal()" type="button">
      Update using signal
    </button>
