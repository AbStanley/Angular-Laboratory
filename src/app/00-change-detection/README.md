...
export class ParentComponent {
  ...
  replaceUser() {
    this.user = { ...this.user, name: "Replaced Stanley" };
  }
}
```
...
export class ParentComponent {
  userSignal = signal({ name: "Stanley" });

  mutate() {
    this.userSignal.update((user) => ({ ...user, name: "Updated Stanley" }));
  }

  replace() {
    this.userSignal.set({ name: "Replaced Stanley" });
  }
}
```
...
@Component({
  ...
  template: `
    <button (click)="changeSomething()">
      Change something (no effect on input)
    </button>
    <app-child [user]="user"></app-child>
  `,
})
...
_❌ What happens when clicking Mutate `user.name`?_
...
_✅ What happens when clicking Replace `user` object?_
